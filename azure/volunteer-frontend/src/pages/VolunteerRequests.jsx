...
export default function VolunteerRequests() {
  ...
  return (
    <>
      ...
      {apps.length === 0 && <div className="card"><p style={{ color: '#999' }}>No pending invitations.</p></div>}
      {apps.map(a => (
        <div key={a.id} className="card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
            <div>
              <h3 style={{ marginBottom: 4 }}>{a.role_name} <span className="badge INVITED">INVITED</span></h3>
              <p style={{ color: '#666', fontSize: 13 }}>{a.event_title} — 📅 {new Date(a.event_date).toLocaleString()} — 📍 {a.location}</p>
            </div>
